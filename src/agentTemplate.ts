export function agentTemplate(
  ...
  // console.log({ templateType }, { vals });

  if (templateType === "batting") {
    return battingTemplate(name, vals);
  }

  if (templateType === "bowling") {
    return bowlingTemplate(name, vals);
  }

  return completeTemplate(name, vals);
}
const battingTemplate = (name: string, vals: number[]) => (`#include <vector>
...
int ccgAgent${name}(Hand hand, Card lastBowledCard, bool isBatting,
  ...
  int battingCriteriaGene[10] = {${vals[0]}, ${vals[1]}, ${vals[2]},
...
const bowlingTemplate = (name: string, vals: number[]) => (`#include <vector>
...
int ccgAgent${name}(Hand hand, Card lastBowledCard, bool isBatting,
  ...
  cardSuit mySuit, bowledSuit;
  int bestCard, bestQuality, quality, whichCard, myNumber, bowledNumber;
  int battingCriteriaGene[10] = {74, 146, 20, 3, 167, 190, 121, 159, 178, 131};

  int bowlingCriteriaGene[8] = {${vals[0]}, ${vals[1]}, ${vals[2]}, 
    ${vals[3]}, ${vals[4]}, ${vals[5]}, ${vals[6]}, ${vals[7]}};
...
//   int bestCard, bestQuality, quality, whichCard, myNumber, bowledNumber;
//
//   int battingCriterionPopulation[10] = {${vals[0]}, ${vals[1]},
//     ${vals[2]}, ${vals[3]}, ${vals[4]}, ${vals[5]},
//     ${vals[6]}, ${vals[7]}, ${vals[8]}, ${vals[9]}};
//
//   int bowlingCriterionPopulation[3] = {100, 100, 100};
//
//   if (isBatting) {
//     // When batting, play the card with the highest quality according to
//     // weighted criteria.
...
//         // highest quality goes to cards of equal number
//         if (myNumber == bowledNumber) {
//           quality += battingCriterionPopulation[0];
//         }
//
//         // if we have a card with a higher number
//         if (myNumber >= bowledNumber) {
//           if (myNumber == getLowestStillSafe(bowledNumber, hand)) {
//             quality += battingCriterionPopulation[1];
//           }
//           // weigh the card according to whether it's the lowest
//           // of its suit in the hand
//           if (myNumber != getLowestOfSuit(mySuit, hand)) {
//             quality += battingCriterionPopulation[2];
//           } else {
//             quality += battingCriterionPopulation[3];
//           }
//         }
//
//         if (myNumber < bowledNumber) {
...
//           // weight to the lowest possible card.
//           if (myNumber == getLowestOfHand(hand)) {
//             quality += battingCriterionPopulation[4];
//           }
//
//           if (mySuit == getMostFrequentSuit(hand, match)) {
//             // give some weight to a card that is lower but
//             // also has the most frequently occurring suit in the hand
//             quality -= battingCriterionPopulation[5];
//             // quality += 5;
//           } else {
//             quality -= battingCriterionPopulation[6];
//           }
//         }
//
//       } else {
...
//         if (myNumber > bowledNumber) {
//           quality +=
//               battingCriterionPopulation[7] * numRuns(myNumber - bowledNumber);
//         } else {
//           if (myNumber == getLowestOfSuit(mySuit, hand)) {
//             // give higher priority to the lowest card of
//             // the same suit
//             quality += battingCriterionPopulation[8];
//           } else {
//             quality += battingCriterionPopulation[9];
//           }
//         }
//       }
//
...
//
//       if ((myNumber != bowledNumber + 1) || myNumber != bowledNumber) {
//         quality += bowlingCriterionPopulation[0];
//       }
//       // be mindful of less favorable matchup
//       if ((myNumber == bowledNumber + 1) || myNumber == bowledNumber) {
//         quality -= bowlingCriterionPopulation[1];
//       }
//       // try to deplete a suit faster
//       if (mySuit == bowledSuit) {
//         quality += bowlingCriterionPopulation[2];
//       }
//
//
//       // Create your own bowling criteria!
...
// }`);
//
const completeTemplate = (name: string, vals: number[]) => (`#include <vector>
...
int ccgAgent${name}(Hand hand, Card lastBowledCard, bool isBatting,
  ...
  int bowlingCriterionPopulation[3] = {${vals[10]}, ${vals[11]}, 
    ${vals[12]}};
